...
When the program is started from the Tcl host after catalogue logon, **`@USERNO`**, **`@ACCOUNT`**, **`@LOGNAME`**, and **`@DEFDATA`** may appear in expressions (same string values as Tcl **`GET`**). They are read-only at compile time and at run time; see [BASIC language — Variables and naming](basic-language.md#variables-and-naming).
