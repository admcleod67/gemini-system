...
Building **`gemini-system`** or **`gemini-daemon`** triggers a **`gemini-bootstrap-copy`** step that copies the entire `gemini/` tree from the source tree into **`CMAKE_CURRENT_BINARY_DIR`/gemini** (typically `<build>/src/gemini` when using the project’s `src/CMakeLists.txt`). That keeps `gemini/` next to the built binaries when the run **working directory** is set to that same directory (e.g. CLion default for targets defined under `src/`). The build also copies language modules (**`libgemini-module-basic`**, **`-pascal`**, **`-comal`**, **`-cobol`**, **`-math`**, **`-stub`**, or platform equivalents) into **`gemini/modules/`** under that same tree.

**Installed tree (Milestone 17 Stage 6):** `cmake --install` with component **`Runtime`** places the same bootstrap under **`${CMAKE_INSTALL_DATADIR}/gemini`** (typically `<prefix>/share/gemini`) and built modules under **`…/gemini/modules/`**. Application and Service recipes install **`Runtime`** alongside the edition binary component — see [Service daemon — Install packaging](daemon.md#install-packaging-service-vs-application).
