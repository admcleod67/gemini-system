...
There is **no second standalone executable** and **no client failover**: **`gemini-console`** attaches only to a running **`gemini-daemon`**; connection failure is an error, not a switch to embedded mode.
