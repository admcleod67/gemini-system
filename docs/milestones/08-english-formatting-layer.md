...
- **Stage 5 — `ID-SUPP` + HELP integration**: small directive plus operator‑facing HELP updates for `LIST` / `SORT` / `SELECT` via the M6 file‑backed HELP. *Status: implemented.* Ships `ID-SUPP` (fields-only data rows; empty line when no projection) and updated builtin + SYSPROG `HELP` topics for `LIST` / `SORT` / `SELECT` with formatting-clause summaries.
- **Stage 6 — `FOOTING` (stretch) + docs polish**: the spec's stretch directive, plus `docs/english.md`, `docs/tcl-shell.md`, and `docs/english-formatting.md`. *Status: implemented.* Ships `FOOTING "text"` with the same `@` substitution rules as `HEADING`, single end-of-report footer without `HEADING`, per-page footers when `HEADING` + `SET PAGE-LENGTH` pagination is active, and operator/docs polish. **Closes Milestone 8.**
