...
- [x] **`LanguageRegistry`** is implemented and populated at boot.
- [x] Dynamic module loader loads and registers modules.
...
- [x] Existing bytecode remains fully compatible.
- [x] Documentation and tooling are updated.
...
- **Stage 1 — Registry foundation**: implement **`LanguageRegistry`** (namespace ID → function table, optional hooks/metadata); stable runtime errors for unknown namespace, unknown function ID, and arity mismatch; unit tests only with an **in‑process test namespace** (no **`dlopen`**, no compiler or **`InvokeBuiltin`** changes). Implementation: [`src/core/languages/`](../../src/core/languages/) (`LanguageTypes.h`, `LanguageRegistry.h`, `LanguageRegistry.cpp`), tests in [`tests/core/test_LanguageRegistry.cpp`](../../tests/core/test_LanguageRegistry.cpp). *Status: implemented.*

- **Stage 2 — `CALL_FUNC` VM + assembler**: add **`CALL_FUNC`** opcode to **`Runtime`**, text assembler (**`Parser`**, **`InstructionPrint`**, **`BytecodeText`**); dispatch through the registry populated with the Stage 1 test namespace; VM/assembler/runtime tests; existing bytecode unchanged. No dynamic loading yet. Implementation: `CallFuncOperand` on [`Instruction`](../../src/core/vm/Runtime.h), tests in [`tests/core/test_CallFunc.cpp`](../../tests/core/test_CallFunc.cpp) and parser round-trip in [`tests/core/test_Parser.cpp`](../../tests/core/test_Parser.cpp). *Status: implemented.*

- **Stage 3 — Dynamic module loader**: boot‑time scan of configurable **`gemini/modules/`** (or host‑configured path); **`dlopen`** / **`LoadLibrary`**; **`register_language(LanguageRegistry&)`** C ABI; failed modules logged, not fatal; registry immutable after boot. Ship a **minimal stub module** proving load + registration. No BASIC migration yet. Implementation: [`LanguageModuleLoader`](../../src/core/languages/LanguageModuleLoader.h), [`LanguageModuleAbi.h`](../../src/core/languages/LanguageModuleAbi.h), [`LanguageRegistry::instance()`](../../src/core/languages/LanguageRegistry.h), stub [`modules/gemini-stub/`](../../modules/gemini-stub/), tests in [`tests/core/test_LanguageModuleLoader.cpp`](../../tests/core/test_LanguageModuleLoader.cpp). *Status: implemented.*
...
- **Stage 6 — Tooling + docs — closes M11**: operator/developer surface to inspect loaded languages (**`SYSTEM LANGUAGES`**, **`SHOW-MODULES`** alias); boot diagnostics retained for module failures; milestone index cross-links; reference BASIC module documented as canonical example. Implementation: [`LanguageRegistry::listNamespaces`](../../src/core/languages/LanguageRegistry.h), [`LanguageModuleBootLog`](../../src/core/languages/LanguageModuleBootLog.h), [`LanguageRegistryFormat`](../../src/core/languages/LanguageRegistryFormat.h), Tcl commands in [`Shell.cpp`](../../src/userland/tcl/Shell.cpp), tests in [`tests/core/test_LanguageRegistryFormat.cpp`](../../tests/core/test_LanguageRegistryFormat.cpp), [`tests/tcl/test_Shell.cpp`](../../tests/tcl/test_Shell.cpp). *Status: implemented. **Closes Milestone 11.***
