...
4. Admin visibility and control over attached sessions (especially runaway CPU-bound programs until [**Milestone 24**](24-execution-fairness-cpu-bound-yield.md))
...
**Privilege:** Admin introspection (**`LISTSESSIONS`**, **`STATUS`**) is restricted to logged-in account **`SYSPROG`**. **`SHUTDOWN`** and **`KILLSESSION`** (Stage 5) use the same gate. Embedded **`gemini-system`** (`maxSessions = 1`) accepts **`STATUS`** / **`LISTSESSIONS`** with one row / trivial summary (`socket=none`) and **`SHUTDOWN`** as process exit.
...
- CPU-bound opcode-budget yield or operator **BREAK** ([**Milestone 24**](24-execution-fairness-cpu-bound-yield.md))
...
- **Stage 2 — Logging / journald readiness**: audit daemon stdout/stderr under non-TTY; ensure cold-start and **`MODULES:`** lines flush; document systemd `StandardOutput=journal` expectations; fix any interactive-only assumptions in the daemon entry path. **Exit criterion:** headless daemon start produces complete boot banner to a pipe/file; documented journald behaviour. *Status: implemented.* Ships line-buffered stdout in [`src/daemon/Main.cpp`](../../src/daemon/Main.cpp), `BootMonitor` flush, **`IPC LISTENING:`** line in [`GeminiDaemonRunner`](../../src/userland/tcl/GeminiDaemonRunner.cpp), lifecycle test, and journald notes in [`docs/daemon.md`](../daemon.md).

- **Stage 3 — systemd unit**: add `gemini.service` (+ install rules); wire `ExecStart` to installed `gemini-daemon` with config path; SIGTERM stop; document `systemctl` operator steps. **Exit criterion:** unit file present and installable; manual systemd smoke starts/stops daemon (CI may mark systemd tests as optional/manual). *Status: implemented.* Ships [`packaging/systemd/gemini.service.in`](../../packaging/systemd/gemini.service.in), [`packaging/gemini/daemon.conf`](../../packaging/gemini/daemon.conf), CMake install rules, [`tests/core/test_SystemdPackaging.cpp`](../../tests/core/test_SystemdPackaging.cpp), and systemd operator notes in [`docs/daemon.md`](../daemon.md).

- **Stage 4 — Admin LISTSESSIONS + STATUS**: implement Tcl **`LISTSESSIONS`** and **`STATUS`** (and optional **`SYSTEM`** aliases) using [`SessionTable`](../../src/userland/tcl/SessionTable.h) / [`CooperativeSessionRunner`](../../src/core/daemon/CooperativeSessionRunner.h) state; privilege gate decided and tested. **Exit criterion:** two-console integration test shows both ports in **`LISTSESSIONS`**; **`STATUS`** reports capacity and count. *Status: implemented.* Ships [`AdminQueries.h`](../../src/userland/tcl/AdminQueries.h), host `listAdminSessions` / `adminStatus`, Shell **`LISTSESSIONS`** / **`STATUS`** / **`SYSTEM STATUS`** (SYSPROG gate), [`tests/tcl/test_AdminCommands.cpp`](../../tests/tcl/test_AdminCommands.cpp), and two-console coverage in [`tests/console/test_GeminiConsole.cpp`](../../tests/console/test_GeminiConsole.cpp).

- **Stage 5 — KILLSESSION + SHUTDOWN + shared teardown**: implement **`KILLSESSION` *port*** (unbind console if any, `retireSession`, `destroySession`, release locks/ports) and Pick-style Tcl **`SHUTDOWN`** that calls the same graceful stop path as **SIGTERM** / IPC **`ShutdownRequest`** (`requestShutdown` on the daemon; process exit on embedded). Privilege-gate both destructive verbs. **Exit criterion:** kill one of two sessions while the other continues; locks held by killed session are gone; **`SHUTDOWN`** from a privileged console stops the daemon cleanly (integration test); embedded **`SHUTDOWN`** exits `gemini-system`. *Status: implemented.* Ships extended [`ShellAdminQueries`](../../src/userland/tcl/AdminQueries.h), runner `killSession` with deferred destroy (avoids join-while-holding-token deadlock), Shell **`KILLSESSION`** / **`SHUTDOWN`** / **`SYSTEM SHUTDOWN`**, unit coverage in [`tests/tcl/test_AdminCommands.cpp`](../../tests/tcl/test_AdminCommands.cpp), and two-console kill + Tcl shutdown tests in [`tests/console/test_GeminiConsole.cpp`](../../tests/console/test_GeminiConsole.cpp).
