...
- **Stage 2 — F‑type in ENGLISH + `RESOLVE-FIELD`**: wire evaluator into ENGLISH field resolution for projection and `SORT BY` keys; `LIST` / `SORT` / `SELECT` / `COUNT` treat F‑type tokens as first‑class fields. Extend **`RESOLVE-FIELD`** to display F‑type definitions and evaluation hints. Unknown fields remain hard errors. Queries without F/I items remain byte‑identical to pre‑M9 output. *Status: implemented.* Ships `CorrelativeEvaluator::evaluateFieldCell`, ENGLISH projection/sort/break/total via F fields, and Tcl `RESOLVE-FIELD` output for F-type DICT items (conversion tail still not evaluated).

- **Stage 3 — F‑type conversion (`CONV`)**: F correlatives invoke the Milestone 7 ICONV/OCONV subset (`F;…;CONV;…`); document supported codes in [docs/correlatives.md](../correlatives.md) (initial stub acceptable). Reject unsupported conversion codes with stable, test‑locked messages. No new conversion codes beyond M7. *Status: implemented.* Ships shared [`PickOconv`](../src/core/conversion/PickOconv.cpp) (M7 OCONV output table), F-type attr 3 as OCONV on source first value, VM `OCONV` builtin refactor, [docs/correlatives.md](../correlatives.md) stub, and `RESOLVE-FIELD` selector text for conversion tails.

- **Stage 4 — I‑type core**: I‑type parser and AST; expression evaluator (arithmetic, field references, coercion). Integrate I‑types into ENGLISH projection and `SORT BY`. Lock errors for type mismatch and division by zero. `IF` / function calls deferred to Stage 5. *Status: implemented.* Ships `DictIItemParser`, `IExpressionParser` / `IExpressionEvaluator`, `ICorrelativeDef`, ENGLISH projection/sort via `evaluateFieldCell`, and Tcl `RESOLVE-FIELD` output for I-type items.
