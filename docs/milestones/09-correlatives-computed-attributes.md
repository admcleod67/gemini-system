...
- **Stage 1 — DICT model + F‑type foundation**: parse F‑type DICT records into structured definitions (not raw strings); introduce **CorrelativeEvaluator** skeleton with F‑expression parser and AST; evaluate minimal F correlatives in isolation (`F;2;3`, `F;2;L`, `F;2;R`). Lock stable errors for invalid / missing correlatives. Extend **DictionaryResolver** to recognise F items (A/S precedence unchanged). Unit tests on `StructuredRecord` only — no ENGLISH/BASIC/TCL integration yet. *Status: pending.*
