...
## Milestone 24 — Execution Fairness: CPU-Bound Cooperative Yield

Extend [**Milestone 15**](15-cooperative-multi-session-execution.md) cooperative scheduling so sessions blocked in **CPU-bound** interpreter work (not only at I/O waits) periodically release the execution token. Preserve the single-interpreter-stack invariant: still no preemptive threading or parallel VM stacks. *Status: planned (deferred; after shared math [M21](21-shared-math-language-module.md), host FS [M22](22-host-filesystem-facade.md), and R83-compat [M23](../compatibility-r83-pick.md) as scheduling allows).*

Formerly numbered Milestone 19, then M21, then M22. Renumbered to **M24** so Apollo-facing [**Milestone 21**](21-shared-math-language-module.md) (shared math) and [**Milestone 22**](22-host-filesystem-facade.md) (host FS façade), plus **Milestone 23** (R83 compatibility gaps), precede fairness. Depends on [**Milestone 18**](18-version-1-gemini-system-service.md) (Version 1.0).
...
M24 adds **voluntary yield inside long-running VM work** so other sessions can acquire the token and make progress, without OS time slices or pthread-per-session execution.

---

### 3. Scope

#### 3.1 Opcode-budget yield (v1 of M24)
...
1. **Opcode-budget yield** — constant N in `runBasicUntilStop`; two-session unit + integration test. **Minimum viable M24.**
...
4. **Docs + closes M24** — daemon/console operator notes; full test suite green.
