...
- [Milestone 21 — Shared Math Language Module (completed)](milestones/21-shared-math-language-module.md) — `math` namespace `CALL_FUNC` for `sqrt`/`sin`/`cos`/`tan`/`arctan`/`ln`/`exp`
- [Milestone 22 — Host Filesystem Façade (planned)](milestones/22-host-filesystem-facade.md) — **next**; host-agnostic FS contract for standalone (and later Pick); unblocks Apollo `file`/`text`
...
- [Milestone 24 — Execution Fairness: CPU-Bound Cooperative Yield (planned, deferred)](milestones/24-execution-fairness-cpu-bound-yield.md) — after M22–M23 as scheduling allows; formerly M19 / M21 / M22