...
**Later alignment:** [Milestone 11 — Multi‑Language Runtime Infrastructure](#milestone-11--multi-language-runtime-infrastructure) introduces **`CALL_FUNC`**, a **`LanguageRegistry`**, and dynamic **`modules/`** loading; at that time, Milestone 7 built‑ins are **migrated** from compiler lowering to registry-backed dispatch **without** changing BASIC source compatibility. Until Milestone 11 ships, Milestone 7 remains lowering-first as described in Goals and Scope above.
...
- **Migrate** BASIC built‑ins delivered in [Milestone 7](#milestone-7--basic-functions--mat-operations) onto the new function‑dispatch mechanism (source compatibility unchanged).
...
- Update **`docs/milestones.md`** (this section and cross‑links from Milestone 7).
